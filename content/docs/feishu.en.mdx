...
title: Feishu install
---

# Feishu Bot

Status: Production ready, supports bot DMs and groups. Uses WebSocket long connection to receive messages.
...
On **Permissions**, click **Batch import** and paste the following JSON to import required permissions in one go:
...
### Message Reply (replyToMode)

In group chats, the bot can reply by quoting the user's original message for clearer context.
...
| Value      | Behavior                                    |
| ---------- | -------------------------------------------- |
| `"off"`    | Do not quote (default for DMs)               |
| `"first"`  | Quote only on the first reply                |
| `"all"`    | Quote on every reply (default for groups)    |

> Note: Message reply cannot be used together with streaming card output (`streaming: true`). When streaming is enabled, replies are shown as cards and do not show the quote.

### Multi-agent Routing

Use `bindings` to connect one Feishu bot to multiple agents. The system routes conversations by user ID or group ID.
...
| `match.channel`    | Channel type; use `"feishu"` here                        |
| `match.peer.kind`  | `"dm"` (DM) or `"group"` (group)                         |
| `match.peer.id`    | User Open ID (`ou_xxx`) or group ID (`oc_xxx`)           |

See [Get group/user IDs](#get-groupuser-ids) for how to get these IDs.
